    ...
    if request.method=='POST':
        ...
        for event in events:
            if isinstance(event, MessageEvent):
                                                   ...
                                                   quick_reply=QuickReply(
                                                       items=[
                                                           QuickReplyButton(
                                                               action=URIAction(
                                                                   label="輸入人數",
                                                                   uri='line://oaMessage/{bid}/?{message}'.format(bid='@523goiva',message=quote(f"{chId}/輸入人數:")),
                                                               )
                                                           )
                                                       ]
                                                   )))
                ...
                elif p_action == "chtdBooking":  #包車寫入，並詢問特殊需求
                                                   ...
                                                   quick_reply=QuickReply(
                                                       items=[
                                                            ...
                                                            QuickReplyButton(
                                                                action=URIAction(
                                                                    label="我想填寫需求",
                                                                    uri='line://oaMessage/{bid}/?{message}'.format(bid='@523goiva',message=quote(f"{oid}/請描述特殊需求(100字內):")),
                                                               )
                                                           )
                                                       ]
                                                   )))
                elif p_action == 'booked': 
                    message.append(TextSendMessage(text='謝謝您，我們將會盡快處理您的包車預約。'))
